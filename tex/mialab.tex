...
\end{equation}
\TODO{as computation time is a performance we evaluate as well, the specs of the used device could be given here: x cores, y gb ram...}
...
\TODO{ref. to specs of computer, given in performance evaluation}
...
\begin{figure}[h]
	...
	\caption{Scatter matrix plot for the seven features on the data set size of 100 samples. Each feature is compared against each other. On the diagonal are the data distribution of every feature. The correlation coefficient between two features is shown in the right upper part. f1-f3: Coordinate features, f4: T1 intensity, f5: T1 gradient, f6: T2 intensity, f7: T2 gradient. }
	\label{scatterplot}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{Feature comparison of single features with different algorithms. Each algorithm was trained on the train set of 70 samples and tested on the 30 samples test data. The algorithm are distinguished by the colour. The dice score of each algorithm is calculated separately for the three tissue types. W is for Wm, G for GM and C for CSF.}
	\label{FeatEval} 
\end{figure}
