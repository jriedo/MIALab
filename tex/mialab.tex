...
Figure~(\ref{FeatEval}) visualizes the influence of each feature group and the used algorithm. The features are divided into groups; \textit{coordinate (x, y, z)}, \textit{intensity (T1, T2)}, and \textit{gradient (T1, T2)}. The first column in this figure is calculated with all the features combined and is considered as a reference. Each column belongs to a single feature group in which the dice of the four algorithms used is visualized. Vertically aligned are dices displayed for WM, GM and CSF.
