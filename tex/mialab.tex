...
The decision forest (DF) is an algorithm for supervised regression and classification tasks. A DF is an ensemble which consists of many decision trees. A schematic example of a decision tree is visualized in Fig.~(\ref{f.df}). Each tree starts from top to bottom with a single root node and splits in two after every node. Each branch represents one feature of the dataset, every feature can be used in a different branch at any level of the tree. The leaf nodes at the bottom and are the resulting predictors. The depth of the trees can be regulated by the chosen number of tree nodes. High numbers of trees improve the accuracy, but aswell increases the computation time. \TODO{complete Mike}
...
The hyperparameters were individually determined by a grid search on the train data for WM, GM, and CSF. Visualized in Fig.~(\ref{f.df_gridsearch}) are the results of the grid search for different number of nodes and trees, marked with a red cross. Unfortunately, it was not possible to achieve the best fit in every of three tissue types with the same hyperparameters. Therefore the chosen result of 160 trees and 3000 nodes was a good trade off.
...
Figure~(\ref{scatterplot}) shows the scatter matrix of all the features. The whole dataset of 100 images was used for this evaluation. On the diagonal are the histograms to visualize the data distribution of each feature. The first three histograms on the diagonal are of the coordinate features and are normally distributed. The fourth and fifth histograms belong to the intensity features and follow a non normal distribution. The last two histograms show the gradient features and are partially normal distributed. The part above the diagonal visualizes the linear correlation between each of the feature with the associated correlation coefficient. The part below the diagonal is redundant to the upper part. 
