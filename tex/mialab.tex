...
\TODO{MM: \textbf{Short} intro to DF, ref to \ref{f.df_gridsearch}, mention 80 trees and 3000 max nodes}
...
Tuning of the random mask (see Sec.~\ref{s.introduction}) turned out to be crucial in order to optimize low dice values of the CSF classification. A first approach to take approximately proportionally equal fractions of WM, GM, and CSF turned out to be suboptimal. Better results were achieved by including roughly equal fractions in terms of absolute number of voxels. The background class owns the highest number of voxels. Those are taken into account with about the same amount of voxels as the other classes too. The fraction of voxels taken into account are 0.004 for WM and CSF, 0.003 for GM, and 0.0003 for background.
