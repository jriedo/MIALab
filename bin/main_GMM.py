...
TRAIN_BATCH_SIZE = 1  # 1..70, the higher the faster but more memory usage
...
def main(_):
    ...
    for batch_index in range(0, len(data_items), TRAIN_BATCH_SIZE):
        ...
        gmm.fit(data_train, labels_train)
    ...
    for batch_index in range(0, len(data_items), TEST_BATCH_SIZE):
        ...
        for img in images_test:
            ...
            start_time = timeit.default_timer()
            probabilities = gmm.predict_proba(img.feature_matrix[0])
            predictions = gmm.predict(img.feature_matrix[0])
